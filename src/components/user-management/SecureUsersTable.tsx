...
import { secureStringRender } from '@/utils/securityHelpers';
...
const SecureUsersTable = ({ users, currentPage, totalPages, currentUser, onPageChange, onEditRole }: UsersTableProps) => {
  ...
  const getRoleBadge = (role: string) => {
    ...
    return <Badge className={colors[role] || 'bg-gray-600'}>{secureStringRender(role)}</Badge>;
  };
            ...
            <TableRow key={userData.id}>
              <TableCell>
                <div className="flex items-center gap-3">
                  {userData.avatar_url && (
                    <img 
                      src={secureStringRender(userData.avatar_url)} 
                      alt={secureStringRender(userData.full_name)}
                      className="w-8 h-8 rounded-full object-cover"
                    />
                  )}
                  <div>
                    <div className="font-medium">{secureStringRender(userData.full_name)}</div>
                    <div className="text-sm text-gray-500">{secureStringRender(userData.email)}</div>
                  </div>
                </div>
              </TableCell>
              ...
              <TableCell>
                <div>
                  <div className="text-sm">{secureStringRender(userData.phone)}</div>
                  <div className="text-sm text-gray-500 max-w-xs truncate">{secureStringRender(userData.address)}</div>
                </div>
              </TableCell>
              <TableCell>{secureStringRender(new Date(userData.created_at).toLocaleDateString())}</TableCell>
              <TableCell>{secureStringRender(new Date(userData.updated_at).toLocaleDateString())}</TableCell>
              <TableCell>
...
export default SecureUsersTable;
